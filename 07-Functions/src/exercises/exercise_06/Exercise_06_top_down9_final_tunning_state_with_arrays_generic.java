import static exercises.exercise_06.helpers.ArrayHelpers.*;
...
Character[] secretWord;
Character[] maskedWord;
Character[] notInSecretWordLetters;
Character[] usedLetters;
int lives;
...
private void initializeNotInSecretWordLetters() {
    notInSecretWordLetters = new Character[0];
}

private void initializeUsedLetters() {
    usedLetters = new Character[0];
}
...
Character[] askSecretWord() {
    String input = IO.readln("Dime la palabra secreta a adivinar: ");
    Character[] secretWord = new Character[input.length()];
    for (int i = 0; i < secretWord.length; i++) {
        secretWord[i] = input.charAt(i);
    }
    return secretWord;
}
...
Character[] createInitialMaskedWord() {
...
Character[] uncoverMaskedLetter(char letter) {
